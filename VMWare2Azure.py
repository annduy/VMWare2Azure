...
#=== Hyper-V VM info
vhd_path= "D:\\HyperV-VMs\\UbuntuServer20046"
vhd_hdds = []
#==== AZ info
az_vm_name = ''
az_hdds =[]
...
def convert_vmdks_2_vhds(vmdk_path,vmdk_hdds,vhd_path):
    try:
        ...
        for vmdk_hdd in vmdk_hdds:
            ...
            print(current_date_time_string() + " : " + f"converting file: {vmdk_path}\{vmdk_hdd}"  )
            ...
            if (hdd_len_in_bytes % (1024*1024)) != 0: 
                ...
                print(current_date_time_string() + " : " + f"Resizing file: {vhd_path}\{vhd_hdd}"  )
...
def upload_vhds_2_az_storage(vhd_path,vhd_hdds):
    ...
    try:
        for vhd in vhd_hdds:
            ...
            print(current_date_time_string() + " : " + f"Uploading file {vhd_path}\{vhd} to Azure Storage")
...
def create_az_disk_from_uploaded_vhd(vhd,os_type='Linux',sku='Standard_LRS'):
    ...
    create_az_disk_cmd = f"az disk create -g {az_resourcegroup} -n {az_disk} --sku {sku} --source https://{az_storage}.blob.core.windows.net/{az_container}/{vhd}"
    print(current_date_time_string() + " : " + f"Creating AZDisk from {vhd}")
...
def create_az_vm_from_bootable_az_disk(vm_name,az_disk, os_type='Linux', size='Standard_B1ls'):
    ...
    creat_az_vm_cmd = f"az vm create -g {az_resourcegroup} -n {vm_name} --attach-os-disk {az_disk} --os-type {os_type} --size {size} --public-ip-sku Standard"
    print(current_date_time_string() + " : " + f"Creating AZ VM from {az_disk}")
...
def attach_non_bootable_disk_to_az_vm(vm_name, az_disk, sku='Standard_LRS'):
    ...
    print(current_date_time_string() + " : " + f"Attaching {az_disk} to {vm_name}")
...
def delete_uploaded_vhds():
    ...
    print(current_date_time_string() + " : " + "Delelting all uploaded VHD(s).")
...
def delete_local_vhds(vhd_path):
    ...
    print(current_date_time_string() + " : " + "Deleting all generated VHD(s).")
